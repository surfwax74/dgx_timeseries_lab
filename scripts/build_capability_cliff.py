...
CAPABILITY_LABELS = {
    ...
    "serve_405b_bf16":                "Serve 405B LLM at full bf16 (no quantization)",
...
DUAL_USE_SCENARIOS = [
    ...
    {
        # THE CAPABILITY-CLIFF ROW. 405B x 2 bytes = ~810 GB of weights.
        #   8x H200 @ 141 GB = 1128 GB -> fits, ~318 GB spare for KV cache
        #   8x H100 @  80 GB =  640 GB -> does NOT fit
        #   8x A100 @  80 GB =  640 GB -> does NOT fit
        # Quoting bf16 rather than INT4 is deliberate: INT4 405B is ~203 GB
        # and fits almost any 8-GPU node, which understates the argument.
        "label": "DGX H200 — LLM-heavy\n(8x H200, Llama 3.1 405B bf16)",
        "n_gpus": 8,
        "uses": ["L"] * 8,
        "llm_capacity": "Llama 3.1 405B bf16 — ~810 GB\nDoes NOT fit 8x H100/A100 (640 GB)",
    ...
    {
        "label": "DGX H200 — balanced (recommended)\n(8x H200, Llama 3.3 70B FP8 + training)",
        "n_gpus": 8,
        "uses": ["L", "T", "T", "T", "T", "T", "T", "T"],
        "llm_capacity": "Llama 3.3 70B FP8 — ~71 GB\n(production agentic, 1 GPU)",
        "training_capacity": "FSDP-7 Sat-TSFM training\n+ Phase 7 explain jobs",
        "concurrent_jobs": 4,
        "color_llm": "#ff8c42",
        "color_train": "#19c37d",
    },
    {
        "label": "DGX H200 — efficient co-pilot\n(8x H200, Gemma 4 26B-A4B MoE + training)",
        "n_gpus": 8,
        "uses": ["L", "T", "T", "T", "T", "T", "T", "T"],
        "llm_capacity": "Gemma 4 26B-A4B MoE — ~14 GB\n(3.8B active; <10% of one GPU)",
